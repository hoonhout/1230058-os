        ...
        for j in range(0, len(WindSpd)):
            for k in range(0, len(WatLev)):
                for l in range(0, len(OpenClose)):
                    ...
                    if os.path.exists(pathRes2 +os.path.sep +  RunID_K +os.path.sep +Tab_K) and l==0:
#                        print Tab_K                               
                        fp=open(pathRes2 +os.path.sep +  RunID_K +os.path.sep +Tab_K) 
...
for i in range(0, len(RunID_avail)):
    if RunID_avail[i][-3:-2]== "N":
        ...
        result_K[np.where(abs(WatLev_ind-WatLev)<0.001),np.where(WindSpd_ind== WindSpd),np.where(Dir_ind == Directions)]=float(WH[i]);
    else:
        if RunID_avail[i][-2:-1]== "O":
            ...
            Dir_ind=int(RunID_avail[i][4:7]);
            WindSpd_ind=int(RunID_avail[i][1:3]);   
            result_O_Open[np.where(abs(WatLev_ind-WatLev)<0.001),np.where(WindSpd_ind== WindSpd),np.where(Dir_ind == Directions)]=float(WH[i]);
        else:
            if RunID_avail[i][7:9] == "Lp":  
                WatLev_ind=(float(RunID_avail[i][9:12]))/100
            else:
            ...
            Dir_ind=int(RunID_avail[i][4:7]);
            WindSpd_ind=int(RunID_avail[i][1:3]);   
            result_O_Closed[np.where(abs(WatLev_ind-WatLev)<0.001),np.where(WindSpd_ind== WindSpd),np.where(Dir_ind == Directions)]=float(WH[i]);    
...
for i in range(6,7):    
    for j in range(0,1):
        ...
        if figEmpty == 0:
            ...
            if j==0:
                fig.savefig(pathOut+os.path.sep+"Dir" + str(Directions[i]) +"_Open.png" , dpi=200)
            else:
                fig.savefig(pathOut+os.path.sep+"Dir" + str(Directions[i]) +"_Closed.png" , dpi=200)
        plt.close(fig);
