...
#        region="OS";
    ...
    RunName=[];
    counter=0;
    ListDir=os.listdir(pathInp);
    for i in range(0,len(ListDir)):    
        if ListDir[i][0]=='D':
            Dir.append(ListDir[i])
            ListRuns=os.listdir(os.path.join(pathInp, ListDir[i]));
            for j in range(0,len(ListRuns)):    
                if ListRuns[j][12:14]==region and os.path.exists(os.path.join(pathInp, ListDir[i], ListRuns[j],ListRuns[j] + '.PRT')):
                    RunName.append(ListRuns[j])                     
                    counter=counter+1; 
    ...
    counter=0;
    # loop through runs
    for i in range(0,len(RunName)):
        Dir2=RunName[i][3:7];
        DirectoryRes2 = os.path.join(pathInp,Dir2, RunName[i])      
...
############# check if TAB, NC, SP1 and SP2 files all exist ###################
        ...
        if Summary[counter,0]==1:
            RunFail1.append(RunName[i]);
############# check if PRINT file has errors ##################################
        check=0;
        
        printFile=os.path.join(DirectoryRes2,RunName[i] + '.PRT');
        fp=open(printFile);
        line=fp.readline();
...
############# check number of iterations ######################################
        ...
        counter=counter+1;
    # Write file 
    f=open(os.path.join(pathOut, 'BulkRes'+ region +'.txt'), 'w')
    f.write("THIS FILE CONTAINS RESULTS OF THE PROCEDURAL CONTROL \n\n")
