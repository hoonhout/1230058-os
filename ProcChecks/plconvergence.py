...
def plconvergence(pathInp, pathOut):
    ...
    filePrint=os.path.join(pathInp[0:-3] + "NZ" + pathInp[len(pathInp)-1], fileName);
...
#        plt.title("$H_{sig}$[m] and $RT_{m01}$[s] testlocatie "+ str(i), fontsize=15)
        ...
        plt.text(0.72, 1, " testlocatie "+ str(i), ha="center", va="bottom", fontsize=15, transform = ax1.transAxes)        
        dT=abs(Results[i-1,1,-1]- Results[i-1,1,-2])
        rT=dT/Results[i-1,1,-1]*100
        plt.text(0.85,0.9, "$\Delta$$H_{abs}$ = " + "%0.3f" % (dH)+ " m", horizontalalignment='center', verticalalignment='center',transform = ax1.transAxes,color="blue")
        plt.text(0.85,0.85, "$\Delta$$H_{rel}$ = " + "%0.3f" % (rH)+ " %", horizontalalignment='center', verticalalignment='center',transform = ax1.transAxes,color="blue")           
        plt.text(0.844,0.8, "$\Delta$$T_{abs}$ = " + "%0.3f" % (dT)+ " s", horizontalalignment='center', verticalalignment='center',transform = ax1.transAxes,color="red")
        plt.text(0.848,0.75, "$\Delta$$T_{rel}$ = " + "%0.3f" % (rT)+ " %", horizontalalignment='center', verticalalignment='center',transform = ax1.transAxes,color="red")        
...
#        fig2=plt.figure(2, figsize=(10,10))
        ...
        plt.text(0.30, 1, "&", ha="center", va="bottom", fontsize=15, transform = ax1.transAxes)
        plt.text(0.44,1,"$RT_{m01}$[s]", ha="center", va="bottom", fontsize=15,color="red", transform = ax1.transAxes)        
        plt.text(0.72, 1, "at testlocatie "+ str(i), ha="center", va="bottom", fontsize=15, transform = ax1.transAxes)        
